...
fr=[np.array(Image.open(f'{A}/candle_{k}_{i}.png')) for k in ('idle','move') for i in range(3)]
base=fr[0]
...
for j,i in enumerate(order): Image.fromarray(out[i],'RGBA').save(f'{A}/candle_flicker_{j}.png')
...
bg[0].save(f'{W}/candle_flicker.gif',save_all=True,append_images=bg[1:],duration=90,loop=0)
sheet([Image.fromarray(o,'RGBA') for o in out],6,4,bg=(40,4,12,255)).save(f'{W}/strip_candle_flicker.png'); print('ok',len(order))
