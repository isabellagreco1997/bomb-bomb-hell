...
for d in ('down','up','left','right'):
    names.append(f'heroine_idle_{d}')
    names+= [f'heroine_cycle_{d}_{j}' for j in range(4)]
for k,n in (('idle',3),('move',3),('hurt',3),('defeated',2)): names+=[f'candle_{k}_{i}' for i in range(n)]
...
for d in ('down','up','left','right'):
    meta['anims'][f'heroine_walk_{d}']=[f'heroine_cycle_{d}_{j}' for j in range(4)]
    meta['anims'][f'heroine_idle_{d}']=[f'heroine_idle_{d}']
